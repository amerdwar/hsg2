...
#include "Merger.h"
XBT_LOG_NEW_DEFAULT_CATEGORY(merger ,"Messages specific for this example");
void Merger::mergeSpilles(vector<spill*>* v) {
...
Merger::Merger(JobInfo* job,string dataNode,string taskName,double mapRecNum,string mbForDn) {
...
Merger::Merger(JobInfo* job, string dataNode, string taskName,string mbForDn) {
	this->mbForDn=mbForDn;
	this->job = job;
	this->groups = job->combineGroups;
	this->ioSortFactor = job->ioSortFactor;
...
int64_t Merger::getNumCombinedRecordes(int64_t q, int64_t n) {
...
void Merger::merge(vector<spill*>* v, int fIndex, int lIndex,bool isLast) {
...
int64_t Merger::combine(int64_t recNum) {
...
void Merger::mergeReduceSpilles(vector<spill*>* v) {
...
void Merger::mergeReduce(vector<spill*>* v, int fIndex, int lIndex) {
	int64_t recNum = 0;
...
int64_t Merger::factory(int64_t a){
	int64_t t=1;
...
int64_t Merger::orders(int64_t n,int64_t q){
	int64_t res=1;
	int64_t t=n;
...
double Merger::combination(int64_t n,int64_t q){
	int res;
	if (q==0)
		return 1;
