...
Chunk* HddMediator::writeCh(int64_t size) {
	...
	Message *writemsg = new Message(msg_type::cl_dn_wr_ch, sender, dataNode->get_name(),
			hdd_Access::hdd_write, ch);


	dataNode->put(writemsg, 0);
	//XBT_INFO("write  ch in %s",thismb->get_name().c_str());
	Message* ackm = static_cast<Message*>(thismb->get());
	//XBT_INFO("write  ch in");
